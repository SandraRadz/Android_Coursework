...
class LabelDrawingView(context: Context, var visionObjects: List<FirebaseVisionObject>) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (item in visionObjects) {
            ...
            Log.d("TAGS INDEX", tags[index])
            Log.d("TAGS INDEX LENGTH", tags[index].length.toString())
            Log.d("TAGS SIZE", tagSize.toString())

            pen.getTextBounds(tags[index], 0, tags[index].length, tagSize)
            val fontSize: Float = MAX_FONT_SIZE * box.width() / tagSize.width()
